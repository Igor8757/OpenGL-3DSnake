...
using namespace glm;
	...
	void IK::makeChange()
	{
		...
		if (!reachable) {
			...
			glm::vec3 Vp = (1 - gamma) * destPosition+ newPositions[0]* gamma;
			...
			shapes[0]->buildAngMatrix(zAxis12, glm::cos(phi), glm::sin(phi*signX));
			...
			// FORWARD :
			newPositions[linksNum] = destPosition;
			newPositions[linksNum - 1] = destPosition;
			float r, gamma;
			...
			// BACKWARD :
			newPositions[0] = glm::vec3(0, 0, 0);
			for (int i = 0; i < linksNum - 1; i++) {
				r = glm::distance(newPositions[i + 1], newPositions[i]);
				gamma = distanceBetweenJoints / r;
				newPositions[i + 1] = (1 - gamma) * newPositions[i] + gamma * newPositions[i + 1];
			}
			IKNewPosTrans();
	...
	void IK::IKNewPosTrans() {
		...
		for (int i = 0; i < linksNum; i++) {
			...
			glm::vec3 Vp = newPositions[i+1]- newPositions[i];
			Zaxis = normalize(Zaxis);
			Xaxis = normalize(Xaxis);
			...
			shapes[i]->buildAngMatrix(zAxis1, glm::cos(phi), glm::sin(phi*signX));
